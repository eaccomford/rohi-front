...
function index({ fetchedData, sermonVideos }) {
  ...
  const sermondata = searchValue
    ? results.map((result) => result.item)
    : fetchedData['page_contents'];
    ...
    <Fragment>
      ...
      <div className="relative h-[300px] sm:h-[400px] lg:h-[500px] xl:h-[600px] md:h-[350px]">
        <Image
          ...
          objectFit="cover"
          className="xblur-sm"
          src={`${process.env.NEXT_PUBLIC_URL}/uploads/small_church1_3798f81f93.jpeg?80751`}
        />
...
export async function getStaticProps() {
  ...
  const sermonVideos = await fetch(
    `${process.env.NEXT_PUBLIC_URL}/pages/4`
  ).then((res) => res.json());
  return {
    props: {
      fetchedData,
